...
class ANETcaptions(object):
    ...
    def __init__(self, ground_truth_filenames=None, prediction_filename=None,
                 ...
                 invalid_video_list_filename=None):
        ...
        if not invalid_video_list_filename == '':
            self.invalid_videos = self.import_invalid_videos(invalid_video_list_filename)
        else:
            self.invalid_videos = []
        ...
        if self.verbose:
            self.scorers = [
                ...
                (Meteor(), "METEOR"),
                # (Rouge(), "ROUGE_L"), # Ignore Rouge score because it's not reported in the paper.
                (Cider(), "CIDEr")
            ]
        else:
    ...
    def import_invalid_videos(self, fname):
        # Reads the invalid file names and makes sure not to include these in the ground truths
        with open(fname) as f:
            content = f.readlines()
        vids = [x.strip() for x in content]
        print("The invalid validation videos are", vids)
        return vids
    ...
    def get_gt_vid_ids(self):
        ...
        for gt in self.ground_truths:
            vid_ids |= set(gt.keys())
        for v in self.invalid_videos:
            vid_ids.remove(v)
            print("Removed video from ground truths: %s." % v)
        return list(vid_ids)
...
def main(args):
    ...
    evaluator = ANETcaptions(ground_truth_filenames=args.references,
                             ...
                             invalid_video_list_filename=args.invalid_video_list_filename)
    evaluator.evaluate()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--invalid_video_list_filename', type=str, default='invalid_video_names_val.txt',
                        help='file containing the list of invalid video names.')
    parser.add_argument('-o', '--output_file_path', type=str, default='output_file_path',
